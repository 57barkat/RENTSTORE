import React, { useContext } from "react";
import { View, Text, TextInput, StyleSheet } from "react-native";
import { FormContext } from "@/contextStore/HostelFormContext";

export default function Step5RulesPolicies() {
  const formContext = useContext(FormContext);
  if (!formContext)
    throw new Error("Step5RulesPolicies must be inside HostelFormProvider");

  const { data, updateForm } = formContext;

  const handleRulesChange = (value: string) => {
    const rules = value
      .split(",")
      .map((r) => r.trim())
      .filter((r) => r.length > 0);

    updateForm("rules", rules);
  };

  return (
    <View style={styles.stepContainer}>
      <Text style={styles.stepTitle}>📜 Hostel Rules & Policies</Text>

      <Text style={styles.label}>Rules</Text>
      <TextInput
        style={[styles.input, styles.multilineInput]}
        placeholder="Enter hostel rules separated by commas"
        value={data.rules?.join(", ") || ""}
        onChangeText={handleRulesChange}
        multiline
        numberOfLines={3}
      />

      <Text style={[styles.label, { marginTop: 10 }]}>Safety Features</Text>
      <TextInput
        style={[styles.input, styles.multilineInput]}
        placeholder="List safety features (e.g. CCTV, Fire alarms, Guards)"
        value={data.safetyFeatures?.join(", ") || ""}
        onChangeText={(value) => {
          const safetyFeatures = value
            .split(",")
            .map((f) => f.trim())
            .filter((f) => f.length > 0);
          updateForm("safetyFeatures", safetyFeatures);
        }}
        multiline
        numberOfLines={3}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  stepContainer: { marginBottom: 25, paddingHorizontal: 10 },
  stepTitle: { fontSize: 20, fontWeight: "bold", marginBottom: 15 },
  label: { fontSize: 14, fontWeight: "500", color: "#444", marginBottom: 6 },
  input: {
    borderWidth: 1,
    borderColor: "#ccc",
    borderRadius: 6,
    padding: 10,
    fontSize: 14,
    marginBottom: 15,
  },
  multilineInput: { minHeight: 50, textAlignVertical: "top" },
});
