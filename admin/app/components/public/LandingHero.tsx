...
const HERO_TRUST_ITEMS = [
  {
    title: "Platform-reviewed",
    description: "Limited listing quality checks",
    icon: ShieldCheck,
    hasTooltip: true,
  },
  {
    title: "Area-first",
    description: "Sectors, universities, and landmarks",
    icon: MapPin,
  },
  {
    title: "Focused filters",
    description: "Budget and property type paths",
    icon: SlidersHorizontal,
  },
  {
    title: "Local inventory",
    description: "Homes, hostels, shops, and offices",
    icon: Building2,
  },
];
...
export default function LandingHero() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#07143f]">
      ...
      <div className="relative mx-auto flex min-h-[620px] max-w-[1500px] flex-col items-center px-4 pb-14 pt-8 text-center sm:min-h-[650px] sm:px-6 sm:pb-16 lg:min-h-[670px] lg:px-8 lg:pt-12">
        ...
        <h1 className="mt-4 max-w-4xl text-[2.35rem] font-extrabold leading-[1.08] text-white sm:text-[3.25rem] lg:text-[4.1rem]">
        ...
        <div className="mt-6 grid w-full max-w-5xl gap-3 text-left sm:grid-cols-2 lg:grid-cols-4">
          {HERO_TRUST_ITEMS.map((item) => {
            ...
            return (
              <div
                key={item.title}
                className="flex min-w-0 items-start gap-2.5 rounded-lg border border-white/10 bg-white/10 px-3 py-3 text-white backdrop-blur"
              >
                ...
                <span className="min-w-0">
                  <span className="flex items-center gap-1.5 text-[13px] font-bold leading-4 text-white">
                    {item.title}
                    {item.hasTooltip ? <VerificationTooltip /> : null}
                  </span>
        ...
        <form
          ...
          className="mt-7 w-full max-w-[1120px] rounded-[1.5rem] border border-white bg-white p-2.5 text-left shadow-[0_36px_105px_-38px_rgba(2,6,23,0.86)] ring-1 ring-white/20"
        >
        ...
        <p className="mt-3 max-w-[1120px] text-center text-xs font-medium text-white/78">
          Start broad, then refine on the listings page.
        </p>
      </div>
    </section>
  );
...
function VerificationTooltip() {
  return (
    <span className="group relative inline-flex">
      <button
        type="button"
        aria-describedby="landing-verification-tooltip"
        className="inline-flex h-5 w-5 items-center justify-center rounded-full text-white/70 transition hover:bg-white/10 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white/55"
      >
        <Info className="h-3.5 w-3.5" />
        <span className="sr-only">Listing review details</span>
      </button>
      <span
        id="landing-verification-tooltip"
        role="tooltip"
        className="pointer-events-none absolute left-1/2 top-[calc(100%+0.5rem)] z-20 w-72 -translate-x-1/2 rounded-lg border border-white/15 bg-slate-950/95 px-3 py-2 text-xs font-normal leading-5 text-white/86 opacity-0 shadow-2xl transition group-hover:opacity-100 group-focus-within:opacity-100"
      >
        Platform-reviewed means limited checks for listing quality and obvious
        spam. It does not guarantee ownership, condition, availability, price,
