...
async function insertProperties() {
  ...
  const collection = db.collection("properties"); // Load JSON
  ...
  const properties = parsed.data; // Convert IDs to ObjectId, checking for the nested structure

  properties.forEach((p) => {
    // 1. Convert the main property _id (the string) to an ObjectId instance
    p._id = new ObjectId(p._id); // 2. Convert the nested ownerId._id field:
